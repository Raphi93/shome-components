 ...
 * - When `expanded` is false the icon is wrapped in a `Tooltip` (text = menu label, position = "right", class = "tooltip-menu").
 ...
 * />
 */
export function SidebarSpecialMenuChange({
    ...
    const wrappedIcon = (icon: IconProp | string, label: string) => {
        ...
        return !expanded
            ? //<TooltipSelf text={label} position="right" className="tooltip-menus">{iconElement}</TooltipSelf>
            (
                <Tooltip>
                    <TooltipTrigger asChild className="tooltip-menus">{iconElement}</TooltipTrigger>
