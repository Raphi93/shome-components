      ...
      case "text":
        return (
          <div key={f.id} className="image-settings">
            <StringInput label={""} value={getTextValue(f)} onChange={(v: string) => setSettings(s => ({ ...s, [f.id]: v }))} />
          </div>
        );
      case "number":
        return (
          <div key={f.id} className="image-settings">
            <NumberInput label={""} value={getNumberValue(f)} onChange={(val: number | null) => setSettings(s => ({ ...s, [f.id]: val ?? 0 }))} min={(f as any).min} max={(f as any).max} step={(f as any).step} />
          </div>
        );
      case "select":
        return (
          <div key={f.id} className="image-settings">
            <Select options={(f as any).options} value={(settings[f.id] as string) ?? null} onChange={(v) => setSettings(s => ({ ...s, [f.id]: v as string }))} searchable={false} placeholder={f.label} />
          </div>
        );
      case "checkbox":
        return (
